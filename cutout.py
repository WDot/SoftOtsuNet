...
class CutOut(nn.Module):
    ...
    def forward(self,x):
        ...
        mask = torch.ones(x.shape,dtype=torch.float32,device=self.device)
        lambdaVal = self.beta.sample([batch_size]).to(self.device)
        cutSizes = torch.squeeze((torch.sqrt(x.shape[2]*(1-lambdaVal))).type(torch.int64))
        ...
        for i in range(batch_size):
            ...
            mask[i,:,startIdx:endIdx,startIdx:endIdx] = 0.0 #Maskout
        return mask